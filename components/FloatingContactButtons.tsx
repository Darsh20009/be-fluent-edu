'use client'

import { MessageCircle, X } from 'lucide-react'
import { useState, useEffect } from 'react'

export default function FloatingContactButtons() {
  const [showLabel, setShowLabel] = useState(false);

  useEffect(() => {
    // Initial entrance after 2s
    const initialTimer = setTimeout(() => setShowLabel(true), 2000);
    
    // Cycle visibility every 10s (show for 5s, hide for 5s)
    const interval = setInterval(() => {
      setShowLabel(prev => !prev);
    }, 5000);

    return () => {
      clearTimeout(initialTimer);
      clearInterval(interval);
    };
  }, []);

  return (
    <div className="fixed bottom-6 right-6 sm:bottom-10 sm:right-10 z-50 flex flex-col items-end gap-3">
      <a
        href="https://api.whatsapp.com/send/?phone=201091515594"
        target="_blank"
        rel="noopener noreferrer"
        className="group relative flex items-center"
      >
        {/* Elegant Label */}
        <div className={`
          mr-4 px-5 py-2.5 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-emerald-100/50 
          transition-all duration-700 ease-out flex items-center gap-3
          ${showLabel ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10 pointer-events-none'}
        `}>
          <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></div>
          <span className="text-[#1F2937] font-bold text-sm tracking-wide rtl:ml-2">تواصل معنا الآن</span>
        </div>

        {/* Minimalist Button */}
        <div className="relative">
          <div className="absolute inset-0 bg-emerald-500/20 rounded-full blur-2xl group-hover:bg-emerald-500/40 transition-all duration-700"></div>
          <div className="relative w-16 h-16 sm:w-18 sm:h-18 bg-gradient-to-br from-[#10B981] to-[#059669] text-white rounded-[24px] flex items-center justify-center shadow-2xl shadow-emerald-500/20 transform transition-all duration-500 hover:scale-105 hover:-translate-y-1 active:scale-95">
            <svg 
              viewBox="0 0 24 24" 
              className="w-8 h-8 sm:w-9 sm:h-9 fill-current drop-shadow-sm"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 0 5.414 0 12.05c0 2.123.554 4.197 1.604 6.01L0 24l6.149-1.613a11.973 11.973 0 005.897 1.559h.005c6.632 0 12.046-5.414 12.05-12.051a11.85 11.85 0 00-3.513-8.514z"/>
            </svg>
          </div>
        </div>
      </a>
    </div>
  )
}
